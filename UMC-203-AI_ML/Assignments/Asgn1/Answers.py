...
W, Obj_Val = My_FLD(train_img, train_label, 2500) # Projectng 5000 samples
...
for t in thresholds:
    pred = np.where(train_img_fld[:, 0] > t, 1, 0)
...
plt.ylabel("Accuracy")
plt.title("Accuracy vs. Threshold for FLD Component 1")
plt.grid()
plt.show()
...
test_img_fld = test_img @ W  

train_img_fld = train_img @ W
class_means = {}

for i in np.unique(train_label):
    class_indices = (train_label == i)
    class_means[i] = np.mean(train_img_fld[class_indices], axis=0)

# Classify test samples by assigning to nearest class mean in FLD space
predicted_labels = []
for sample in test_img_fld:
    distances = {c: np.linalg.norm(sample - class_means[c]) for c in class_means}
    predicted_labels.append(min(distances, key=distances.get))  # Assign nearest class
